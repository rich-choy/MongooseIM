...
config_spec() ->
    #section{
       items = #{<<"server_user">> => #option{type = binary,
                                              validate = non_empty}},
    ...
    case should_notify_for_jid(UserJid) of
        ...
        false ->
            ?LOG_DEBUG(#{what => presence_notify_filtered,
                        ...
                        reason => <<"JID does not match flux.char.* or flux.sim.* pattern">>})
    end,
    {ok, Acc}.
...
should_notify_for_jid(UserJid) ->
    LocalPart = jid:to_luser(UserJid),
    case LocalPart of
        <<"flux.char.", _/binary>> -> true;
...
send_presence_notification(HostType, UserJid, PresenceType, _Acc) ->
    try
        ...
        ServerJid = jid:make_noprep(ServerUser, jid:to_lserver(UserJid), <<>>),
        ...
        % Create accumulator for routing - route FROM user TO server
        NotificationAcc = mongoose_acc:new(#{
            location => #{location => ?MODULE, line => ?LINE},
            lserver => jid:to_lserver(UserJid),
            element => Notification,
            from_jid => jid:to_bare(UserJid),
            to_jid => ServerJid
        }),

        % Route the notification to the server user using modern interface
        ejabberd_router:route(jid:to_bare(UserJid), ServerJid, NotificationAcc),
...
get_server_user(HostType) ->
    gen_mod:get_module_opt(HostType, ?MODULE, server_user).
